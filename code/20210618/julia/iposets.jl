...
"""
Operations on multiple iposets:
glue
parallel
"""
...
function glue(g::Iposet, p::Iposet)
    ...
    #add edges from g to p
    @inbounds for v1 in vertices(g.poset)
        if !(v1 in g.t)
...
"""
function Iposet(line::String; hex=true)
    oldformat = false
...
function eachiposet(filename::String; hex=true)
    return (Iposet(x, hex=hex)
            for x in eachline(filename) if !occursin(r"^\s*(?:#|$)", x))
end

"""Load array of iposets from file."""
function loadiposets(filename::String; hex=true)
...
function loadiposets!(res::Array{Iposet}, filename::String; hex=true)
    for ip in eachiposet(filename, hex=hex)
        push!(res, ip)
    end
end
